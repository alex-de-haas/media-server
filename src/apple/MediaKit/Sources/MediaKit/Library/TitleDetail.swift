...
public struct TitleTrack: Identifiable, Equatable, Sendable {
    ...
    /// What the probe said this stream's dynamic range is — `SDR`, `HDR10`, `Dolby Vision`, or the
    /// generic `HDR` a container header cannot be more precise than. Carried because a refusal saying
    /// "unsupported dynamic range" is unactionable without it, exactly as one about a codec is.
