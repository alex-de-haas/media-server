...
test("failed history deletion retains the confirmation and the viewing", async ({ page }) => {
  ...
  await page.route("**/api/proxy/api/library/m1/watch-history?*", (route) => route.fulfill({ json: {
    entries: [{ id: "e1", watchedAt: dates[0] }, { id: "e2", watchedAt: dates[1] }], undated: [], total: 2, datedTotal: 2, offset: 0, limit: 20,
  } }));
  await page.route("**/api/proxy/api/watch-history/entries/e1", (route) => route.fulfill({ status: 500, json: { error: "Try again" } }));
  await page.goto("/movies/m1");
  await page.getByRole("button", { name: /Delete viewing/ }).first().click();
  await page.getByRole("button", { name: "Delete", exact: true }).click();
  await expect(page.getByText("Couldn’t delete this play", { exact: true })).toBeVisible();
  await expect(page.getByRole("alertdialog")).toBeVisible();
  await page.getByRole("button", { name: "Cancel", exact: true }).click();
  await expect(page.getByRole("region", { name: "Watch history" }).getByRole("listitem")).toHaveCount(2);
});
...
test("collection loads despite recommendation failure and its cards open movie details", async ({ page }) => {
  ...
  await expect(page.getByRole("region", { name: "Watch history" })).toHaveCount(0);
  await expect(page.getByRole("heading", { name: "More from Saga" })).toBeVisible();
...
test("movie history stays usable on a narrow viewport", async ({ page }, testInfo) => {
  ...
  await setupApp(page, { detail: { m1: movieDetail("m1", "Arrival") } });
  await page.route("**/api/proxy/api/library/m1/watch-history?*", (route) => route.fulfill({ json: {
    entries: dates.map((watchedAt, index) => ({ id: `e${index}`, watchedAt })),
    undated: [], total: dates.length, datedTotal: dates.length, offset: 0, limit: 20,
  } }));
  await page.goto("/movies/m1");
  const history = page.getByRole("region", { name: "Watch history" });
  const watches = history.getByRole("list", { name: "Dated watches" });
  await expect(watches.getByRole("listitem")).toHaveCount(3);
  const firstWatch = await watches.getByRole("listitem").first().boundingBox();
  const lastWatch = await watches.getByRole("listitem").last().boundingBox();
  expect(lastWatch!.y).toBe(firstWatch!.y);
  expect(lastWatch!.x).toBeGreaterThan(firstWatch!.x);
  expect(await page.evaluate(() => document.documentElement.scrollWidth)).toBeLessThanOrEqual(390);
  const lastEdit = watches.getByRole("button", { name: /Change time for/ }).last();
...
test("related recommendations appear before tags and navigate to available titles", async ({ page }) => {
  ...
  await expect(page.locator("main h2")).toHaveText([
    "Cast", "Media", "Similar movies in your library", "Tags",
  ]);
