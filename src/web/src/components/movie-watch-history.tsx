...
export function MovieWatchHistory({ id, title, removed }: { id: string; title: string; removed: boolean }) {
  ...
  const rows = [...(expanded ? dated : dated.slice(0, 3)), ...(first?.undated ?? [])];

  return (
    <section aria-label="Watch history" className="flex flex-col gap-3">
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <Button variant="ghost" size="sm" onClick={() => setEditing("new")}>
          <CalendarPlus className="size-4" aria-hidden /> Log watch
        </Button>
      </div>
      ...
      <ul className="divide-y">
        {rows.map((entry) => (
          <li key={entry.id} className="flex items-center justify-between gap-3 py-1.5 text-sm">
            {entry.watchedAt ? <time dateTime={entry.watchedAt}>{watchLabel(entry)}</time> : <span>Date unknown</span>}
