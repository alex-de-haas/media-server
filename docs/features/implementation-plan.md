...
| --- | --- | --- | --- |
| External catalog-root mounts (#1) | **Implemented** | `externalMounts.catalogRoots` (`kind: host-path`, `multiple`, `mode: rw`, `service`, `required`). Injected `HOSTY_MOUNT_CATALOGROOTS` as comma-joined `label=path`: docker → `{label}=/mnt/catalogRoots/{label}` (one bind per path), dev → `{label}=host-path`. | **Unblocks `docker`.** One root = one bind = one filesystem, so the move from `.incoming/` into the canonical tree is atomic. The `label` is forwarded to the torrent-engine (`mountLabel`) to pick the matching downloads mount. |
...
[catalogs](catalogs/feature.md), [metadata](metadata/feature.md), [background-tasks](background-tasks/feature.md).
