...
1. **`native-client-api`** *(server)* — the `/native/v1` surface: device-code
   pairing against Hosty Core instead of the Jellyfin PIN credential; cursor-based
...
- **Do sidecar audio tracks survive packaging?** They are separate files; ffmpeg
  ...
  is unverified.
- **Pairing UX on tvOS** — device code entered in the web UI, or approval pushed
  from an already-signed-in iPhone? The second is nicer and needs more moving
  parts.
