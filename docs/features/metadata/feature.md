...
- Metadata blobs are stored as JSON in the database (see
  [Storage and data](../storage-and-data/feature.md)); image binaries are cached on disk
...
- Catalog-wide refresh re-runs the idempotent enrich for **every identified item**
  ...
  over the realtime job feed (see [Background tasks](../background-tasks/feature.md)); only one
